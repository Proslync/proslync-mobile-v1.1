...
export function predictClearance(input) {
  ...
  if (gate.verdict === 'likely-rejected') {
    ...
    if (failingTest && failingTest[0] === 'businessPurpose') {
      ...
      // The reason must describe the FAILING test, not whatever flag sorts
      // first. A deal can be both an associated entity AND over-comp; in that
      // case the compRange failure must report the comp-out-of-range detail.
      const compFlag = gate.flags.find((f) => f.kind === 'comp-out-of-range');
      reason = (compFlag && compFlag.detail) ? compFlag.detail : 'Deal amount exceeds 3× the FMV comp-range high — CSC rejection risk is elevated.';
    } else {
      const anyFlag = gate.flags.find((f) =>
        f.kind === 'comp-out-of-range' || f.kind === 'associated-entity' || f.kind === 'no-comp-data',
      );
      reason = (anyFlag && anyFlag.detail) ? anyFlag.detail : 'One or more CSC gate tests failed.';
    }
  } else if (fmvApplies && amountCents > 3 * fmv.highCents) {
    // Independent price backstop. Today scorePreclearance also fails the
    // compRange test at >3× (because we feed it the FMV range), so this is
    // normally reached via the gate branch above. Keep this branch as a
    // deliberate safeguard: if the FMV range is ever NOT wired into the gate,
    // >3× must still resolve to 'unlikely'. Do not remove.
    band = 'unlikely';
