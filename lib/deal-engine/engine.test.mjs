...
test('deriveDealNotifications: response-due fires at EXACTLY 48h, not at 48h+1s', () => {
  const now = '2026-06-10T00:00:00.000Z';
  const mk = (deadline) => ({
    dealId: 'D1',
    milestones: [{
      id: 'm1', status: 'disputed', description: 'x',
      dispute: { athleteResponse: null, athleteResponseDeadlineISO: deadline },
    }],
    events: [],
  });
  // exactly 48h ahead → included (boundary is `<= 48`)
  assert.ok(
    deriveDealNotifications([mk('2026-06-12T00:00:00.000Z')], now).some((n) => n.kind === 'response-due'),
    'exactly 48h should produce a response-due notification',
  );
  // just over 48h → excluded
  assert.ok(
    !deriveDealNotifications([mk('2026-06-12T00:00:01.000Z')], now).some((n) => n.kind === 'response-due'),
    'past 48h should not produce a response-due notification',
  );
  // deadline already passed (hoursRemaining <= 0) → excluded
  assert.ok(
    !deriveDealNotifications([mk('2026-06-10T00:00:00.000Z')], now).some((n) => n.kind === 'response-due'),
    'an already-passed deadline should not produce a response-due notification',
  );
});

test('deriveDealNotifications: response-due suppressed once athlete has responded', () => {
  const now = '2026-06-10T00:00:00.000Z';
  const deal = {
    dealId: 'D2',
    milestones: [{
      id: 'm1', status: 'disputed', description: 'x',
      dispute: { athleteResponse: 'I dispute this', athleteResponseDeadlineISO: '2026-06-12T00:00:00.000Z' },
    }],
    events: [],
  };
  assert.ok(
    !deriveDealNotifications([deal], now).some((n) => n.kind === 'response-due'),
    'no response-due once the athlete has already responded',
  );
});
