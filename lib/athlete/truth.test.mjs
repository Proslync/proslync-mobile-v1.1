...
// ── urgencyForDeadline ─────────────────────────────────────────────────────
// Regression: the home/deals/truth-strip surfaces coloured a deal's deadline
// via thresholdForHours(hoursUntilISO(iso)). hoursUntilISO clamps a PAST
// deadline to null, and thresholdForHours(null) is 'green' — so an OVERDUE NIL
// Go disclosure (the hardest eligibility consequence, ranked MOST urgent by
// nextDisclosureDeadline) rendered as calm green, and an overdue deliverable
// chip was dropped entirely. urgencyForDeadline distinguishes overdue (red)
// from genuinely-no-deadline (green).

test('urgencyForDeadline: an OVERDUE deadline is red, not green', () => {
  const overdue = new Date(Date.now() - 6 * 3600e3).toISOString(); // 6h past
  assert.equal(urgencyForDeadline(overdue), 'red');
  // Contrast with the old (buggy) path that buried it as calm-green:
  assert.equal(thresholdForHours(hoursUntilISO(overdue)), 'green');
});

test('urgencyForDeadline: exactly-now (0h remaining) counts as overdue → red', () => {
  const now = new Date(Date.now()).toISOString();
  assert.equal(urgencyForDeadline(now), 'red');
});

test('urgencyForDeadline: <24h future is red, 24-72h amber, >72h green', () => {
  const inHours = (h) => new Date(Date.now() + h * 3600e3).toISOString();
  assert.equal(urgencyForDeadline(inHours(10)), 'red');
  assert.equal(urgencyForDeadline(inHours(48)), 'amber');
  assert.equal(urgencyForDeadline(inHours(200)), 'green');
});

test('urgencyForDeadline: no deadline / unparseable → green (nothing owed)', () => {
  assert.equal(urgencyForDeadline(null), 'green');
  assert.equal(urgencyForDeadline(undefined), 'green');
  assert.equal(urgencyForDeadline(''), 'green');
  assert.equal(urgencyForDeadline('not-a-date'), 'green');
});

test('urgencyForDeadline: boundary parity with thresholdForHours for future times', () => {
  // 73h out should match thresholdForHours(73) = green; 23h → red; 25h → amber.
  const inHours = (h) => new Date(Date.now() + h * 3600e3).toISOString();
  assert.equal(urgencyForDeadline(inHours(73)), thresholdForHours(73));
  assert.equal(urgencyForDeadline(inHours(23)), thresholdForHours(23));
  assert.equal(urgencyForDeadline(inHours(25)), thresholdForHours(25));
});
