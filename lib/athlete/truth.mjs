...
// ── Deadline urgency resolver (overdue-aware) ──────────────────────────────

/**
 * Resolve the urgency colour for a deal that HAS a hard deadline (a NIL Go
 * disclosure clock, a deliverable due date) directly from its ISO string.
 *
 * Why this exists separately from `thresholdForHours(hoursUntilISO(iso))`:
 * `hoursUntilISO` deliberately clamps a *past* deadline to `null` (it answers
 * "how long until X?" and there is no positive answer once X has passed). But
 * feeding that `null` into `thresholdForHours` yields 'green' — the SAME calm
 * colour as a genuinely-deadline-less item. For an OVERDUE NIL Go disclosure —
 * the hardest eligibility consequence in the charter get-cleared loop, and the
 * item `nextDisclosureDeadline` already ranks as MOST urgent — calm-green is
 * exactly backwards. It must read RED.
 *
 * This resolver distinguishes the two cases using the SIGNED delta:
 *   - no deadline at all (null/empty/unparseable) → 'green' (nothing owed)
 *   - deadline already passed (overdue)           → 'red'
 *   - otherwise                                   → thresholdForHours(hours)
 *
 * @param {string|null|undefined} deadlineISO
 * @returns {'red'|'amber'|'green'}
 */
export function urgencyForDeadline(deadlineISO) {
  if (!deadlineISO) return 'green';
  const t = new Date(deadlineISO).getTime();
  if (Number.isNaN(t)) return 'green';
  const hours = (t - Date.now()) / (1000 * 60 * 60);
  if (hours <= 0) return 'red'; // overdue — the eligibility consequence
  return thresholdForHours(hours);
}
