...
import type { TicketTier, PricingRule, ValidatePromoCodeResponse } from '@/lib/types/pricing.types';
import type { CreatePaymentIntentResponse, PaymentBreakdown } from '@/lib/types/payments.types';
...
export function PurchaseTicketSheet({
  ...
  const [selectedPricingId, setSelectedPricingId] = React.useState<number | null>(null);
  const [quantity, setQuantity] = React.useState(1);
  const [promoCode, setPromoCode] = React.useState('');
  const [appliedPromoCode, setAppliedPromoCode] = React.useState<ValidatePromoCodeResponse | null>(null);
  const [paymentIntent, setPaymentIntent] = React.useState<CreatePaymentIntentResponse | null>(null);
  const [errorMessage, setErrorMessage] = React.useState<string | null>(null);

  // React Query hooks
  const { data: tiers, isLoading: isLoadingTiers, error: tiersError } = useGetTiers(eventId);
  ...
  const selectedPricing = selectedTier?.pricing.find((p) => p.id === selectedPricingId);
  ...
  const isPricingAvailable = React.useCallback((pricing: PricingRule): boolean => {
    ...
    if (tiers && tiers.length > 0 && !selectedTierId) {
      ...
      if (firstAvailableTier) {
        ...
        const defaultPricing =
          firstAvailableTier.currentPricing || firstAvailableTier.pricing.find(isPricingAvailable);
        if (defaultPricing) {
  ...
  React.useEffect(() => {
    if (selectedTierId && tiers) {
      ...
      if (tier) {
        const defaultPricing = tier.currentPricing || tier.pricing.find(isPricingAvailable);
    ...
    <View style={styles.section}>
      ...
      <View style={styles.tierList}>
        {tiers?.map((tier) => {
          ...
          const isAvailable = tier.isActive && tier.pricing.some(isPricingAvailable);
            ...
            >
              ...
              {tier.currentPricing && (
                <Text style={[styles.tierPrice, { color: colors.text }]}>
                  {formatPrice(tier.currentPricing.price, tier.currentPricing.currency)}
                </Text>
              )}
              {!isAvailable && (
                <Text style={[styles.soldOutText, { color: colors.textTertiary }]}>Sold Out</Text>
              )}
            </TouchableOpacity>
          );
        })}
  ...
  const renderPricingOptions = () => {
    if (!selectedTier || selectedTier.pricing.filter(isPricingAvailable).length <= 1) {
      return null;
    }

    return (
      <View style={styles.section}>
        <Text style={[styles.sectionTitle, { color: colors.text }]}>Select Pricing</Text>
        <View style={styles.pricingList}>
          {selectedTier.pricing
            .filter(isPricingAvailable)
            .map((pricing) => {
      ...
      <View style={styles.promoRow}>
        ...
        {appliedPromoCode ? (
          ...
          <TouchableOpacity
            ...
            disabled={!promoCode.trim() || !selectedTierId || validatePromoMutation.isPending}
            style={[
              styles.promoButton,
              {
          ...
          >
            {validatePromoMutation.isPending ? (
              ...
              <Text style={[styles.promoButtonText, { color: colors.text }]}>Apply</Text>
            )}
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.quantityRow}>
          ...
          <View style={styles.quantityDisplay}>
            ...
            {availableQuantity > 0 && (
              <Text style={[styles.availableText, { color: colors.textTertiary }]}>
    ...
    <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      {isLoadingTiers ? (
        ...
        <View style={styles.emptyContainer}>
          <Ionicons name="alert-circle-outline" size={32} color={colors.textTertiary} style={{ marginBottom: 8 }} />
          <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
            Failed to load tickets
          </Text>
          <Text style={[styles.emptyText, { color: colors.textTertiary, fontSize: 12, marginTop: 4 }]}>
        ...
        <>
          ...
          >
            {createPaymentMutation.isPending ? (
              ...
              <>
                <Ionicons name="ticket-outline" size={20} color={colors.text} style={{ marginRight: 8 }} />
